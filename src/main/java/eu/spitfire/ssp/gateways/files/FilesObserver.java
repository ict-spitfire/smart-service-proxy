...
public class FilesObserver extends AbstractResourceObserver implements Runnable{
    ...
    private FilesGatewayManager serviceManager;
    private Path observedDirectory;

    /**
     * @param serviceManager The {@link FilesGatewayManager} that is responsible for registration and maintainance of
     ...
     * @param localChannel the {@link LocalServerChannel} to send internal messages, e.g. for resource registration
     *
     * @throws IOException if some I/O error occurred during the creation of the observation services
     */
    public FilesObserver(FilesGatewayManager serviceManager, Path directory,
    ...
    private void handleFileCreationOrModification(final Path filePath){
        ...
        Map<URI, Model> resourcesFromFile = FilesProxyTools.readModelsFromFile(filePath);
