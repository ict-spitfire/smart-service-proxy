...
public class VirtualSensorCreator extends AbstractVirtualSensorCreator {
    ...
    protected void processPost(final Channel channel, final HttpRequest httpRequest,
        ...
        URI graphName = new URI(((MixedAttribute) decoder.getBodyHttpData("graphName")).getValue());
        ...
        if("btnTest".equals(buttonID)){
            ...
            handlePostCreationRequest(httpResponseFuture, graphName, model, sparqlQuery, httpRequest.getProtocolVersion());
        }
    }
    ...
    private ListenableFuture<Model> addSensorValueToModel(final Query sensorValueQuery, final Model model){
        final SettableFuture<Model> modelFuture = SettableFuture.create();

        Futures.addCallback(executeSparqlQuery(sensorValueQuery), new FutureCallback<ResultSet>() {
            @Override
            ...
            @Override
            public void onFailure(Throwable t) {
                log.warn("Exception while executing SPARQL query: {}", sensorValueQuery, t);
