...
//
///**
//* A {@link FilesObserver____OLD} observes a given directory to be aware of creation, modification, and deletion of
//* files_OLD containing semantic data or directories.
//*
//* Currently, only N3 is supported as serialization type contained in the files_OLD. A file may contain one or multiple
//* resources (i.e. subjects). However, one proxy service is created for each resource contained in the
//* file.
//*
//* Since the {@link FilesObserver____OLD} only knows the file but not the resource whose status was changed, it is recommended
//* to have only one resource per file since the {@link FilesObserver____OLD} tries to
//* update every resource contained in a file whenever that file changes.
//*
//* @author Oliver Kleine
//*/
//public class FilesObserver____OLD extends DataOriginObserver implements Runnable{
//
...
//     *                       resources backed by local files_OLD
//     * @param directory The {@link Path} representing the root-directory whose files_OLD and sub-directories are to be
//     *                  observed
...
//     */
//    public FilesObserver____OLD(FilesBackendComponentFactory serviceManager, Path directory,
...
//
//    /**
//     * Returns a {@link Collection} containing all files_OLD (no directories) in all observed (sub-)directories
//     * @return a {@link Collection} containing all files_OLD (no directories) in all observed (sub-)directories
//     */
