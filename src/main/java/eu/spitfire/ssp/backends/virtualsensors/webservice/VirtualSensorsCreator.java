 ...
 */
public class VirtualSensorsCreator extends HttpAbstractVirtualSensorCreator {

    private Logger log = LoggerFactory.getLogger(this.getClass().getName());
    private LocalServerChannel localChannel;
    private VirtualSensorRegistry registry;

    public VirtualSensorsCreator(VirtualSensorBackendComponentFactory componentFactory){
        super(
                componentFactory,
    ...
    protected void processPost(final Channel channel, final HttpRequest httpRequest,
        ...
        Model model = createModel(((MixedAttribute) decoder.getBodyHttpData("triples")).getValue());

        String buttonID = ((MixedAttribute) decoder.getBodyHttpData("button")).getValue();

        if("btnTest".equals(buttonID)){
            handlePostTestRequest(httpResponseFuture, model, sparqlQuery, httpRequest.getProtocolVersion());
        }
    ...
    private Model createModel(String triples){
        Model model = ModelFactory.createDefaultModel();
        model.read(new ByteArrayInputStream(triples.getBytes(Charset.forName("UTF8"))), null, Language.RDF_N3.lang);
        return model;
    }

    private void handlePostCreationRequest(final SettableFuture<HttpResponse> httpResponseFuture, final URI graphName,
                                           Query sparqlQuery, final HttpVersion httpVersion) throws Exception{

        final VirtualSensor virtualSensor = new VirtualSensor(graphName, sparqlQuery);

        Futures.addCallback(this.registry.registerVirtualSensor(virtualSensor), new FutureCallback<Void>() {
            @Override
            public void onSuccess(Void result) {
                try{

                    ChannelBuffer content = ChannelBuffers.wrappedBuffer(
                            virtualSensor.getGraphName().toASCIIString().getBytes(Charset.forName("UTF-8"))
                    );

                    HttpResponse httpResponse = HttpResponseFactory.createHttpResponse(
                            httpVersion, HttpResponseStatus.CREATED, content, "text/html"
                    );
    ...
    private void handlePostTestRequest(final SettableFuture<HttpResponse> httpResponseFuture, final Model model,
        ...
        Futures.addCallback(sparqlResultFuture, new FutureCallback<ResultSet>() {
            ...
            public void onSuccess(ResultSet resultSet) {
                ...
                try{
                    ...
                    Resource subject = model.listStatements().nextStatement().getSubject();
                    subject.addProperty(
                            model.createProperty("http://spitfire-project.eu/ontology/ns/value"), sensorValue
                    );
...
//            throws Exception{
//
//        VirtualSensor dataOrigin = new VirtualSensor(formData.getGraphName(), formData.getSparqlQuery());
//
