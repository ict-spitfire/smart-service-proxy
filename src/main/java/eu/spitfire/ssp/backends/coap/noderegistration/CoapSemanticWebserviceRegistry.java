...
import eu.spitfire.ssp.backends.utils.DataOriginRegistry;
...
public class CoapSemanticWebserviceRegistry extends DataOriginRegistry<URI> {
    ...
    public CoapSemanticWebserviceRegistry(CoapBackendManager backendManager) {
    ...
    public void processRegistrationRequest(final SettableFuture<CoapResponse> registrationResponseFuture,
        ...
        try{
            ...
            serviceDiscoveryFuture.addListener(new Runnable(){
                ...
                public void run() {
                    try {
                        ...
                        CoapSemanticWebserviceRegistry.this.backendManager
            ...
            ((CoapBackendManager) backendManager).getCoapClientApplication()
    ...
    private class CoapResourcesRegistrationTask implements Runnable{
        ...
        private void registerTubsResources(final SettableFuture<CoapResponse> registrationResponseFuture)
            ...
            URI rdfServiceUri = new URI("coap", null, remoteAddress.getAddress().getHostAddress(), -1,
                    "/rdf", null, null);

            final ListenableFuture<Map<URI, Boolean>> resourceRegistrationResultFuture =
                    registerResourcesFromDataOrigin(rdfServiceUri,
                            ((CoapBackendManager) backendManager).getHttpRequestProcessor());
                        ...
                        for(String servicePath : coapServices){
                            if(servicePath.endsWith("_minimal")){
                                ...
                                CoapWebserviceObserver observer =
                                        new CoapWebserviceObserver(backendManager, coapRequest);
...
//            backendManager.getCoapClientApplication().writeCoapRequest(coapRequest, responseProcessor);
//
